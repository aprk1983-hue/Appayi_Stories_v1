...
//     // CHANGED: Unified vibrant color for everyone (Vibrant Orange/Amber)
...
//                     // CHANGED: Cartoonish font style and reduced size
//                     style: TextStyle(
//                         color: nameColor,
//                         fontWeight: FontWeight.w900,
//                         fontFamily: 'Fredoka', // Kid-friendly font family
//                         letterSpacing: 1.2,
//                         shadows: [
//                           BoxShadow(
...
//                       _FreeStoriesRow(langCode: langCode),
//                     ],
//                     Padding(
...
//   int _shareIdFromData(Map<String, dynamic> data) {
//     final v = data['shareId'] ?? data['shareID'] ?? data['share_id'] ?? data['storyNo'] ?? data['story_no'];
//     if (v is int) return v;
//     if (v is num) return v.toInt();
...
//       builder: (context, snap) {
//         // While loading config, show a lightweight loader (no scary warnings).
...
//                         ),
//                       ),
//                       const _BlinkingFreeBadge(text: '10 free'), // CHANGED: 20 -> 10
//                     ],
//                   ),
//                 ),
//                 const SizedBox(height: 185, child: Center(child: CircularProgressIndicator())),
//               ],
//             ),
//           );
//         }
...
//         if (ids.isEmpty) {
//           // If config can't be read (rules, etc), hide the whole row (no warning).
//           return const SizedBox.shrink();
//         }

//         final idList = ids.toList()..sort();
//         // CHANGED: Cap the list to max 10
//         final limitedIds = idList.take(10).toList();

//         // Firestore whereIn supports max 10 values.
//         final q1 = FirebaseFirestore.instance.collection('stories').where('shareId', whereIn: limitedIds);
//         // q2 removed as we max out at 10

//         List<Story> _toStories(List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) {
//           final stories = docs.map((d) => Story.fromFirestore(d)).toList();
//           final byShareId = <int, Story>{
//             for (final d in docs)
//               _shareIdFromData(d.data()): Story.fromFirestore(d),
//           };
//           // Keep the same order as in freeShareIdsByLanguage.
//           final ordered = <Story>[];
//           for (final sid in limitedIds) { // Use limitedIds here
//             final s = byShareId[sid];
//             if (s != null) ordered.add(s);
//           }
//           return ordered;
//         }

//         // Calculate dynamic size exactly matching horizontal stories logic
...
//           final stories = _toStories(docs);
//           if (stories.isEmpty) return const SizedBox.shrink();

//           return ListView.separated(
//             scrollDirection: Axis.horizontal,
//             padding: const EdgeInsets.only(right: 16), // Start flush left, pad right only
//             itemCount: stories.length,
//             separatorBuilder: (_, __) => const SizedBox(width: 14),
//             itemBuilder: (context, i) {
//               return _StoryTile(
//                 story: stories[i],
//                 width: itemSize, // Use dynamic size same as popular/horizontal
//                 dark: true,
//                 aspectRatio: 1.0, // SQUARE
//                 imageFit: BoxFit.cover, // Ensure image fills area for rounded edges
...
//                       ),
//                     ),
//                     _BlinkingFreeBadge(text: '${limitedIds.length} free'), // Use limited length
//                   ],
//                 ),
//               ),
//               SizedBox(
//                 height: itemSize, // Container height matches tile size exactly
...
//                   alignment: Alignment.topCenter // Image aligned to top to prevent text cut-off
//                 )
//               ),
//               Positioned(
//                 top: 12,
...
//   const _CoverImage({super.key, required this.src, this.fit = BoxFit.cover, this.alignment = Alignment.center});
...
//         return DecoratedBox(
//           decoration:
//               BoxDecoration(image: DecorationImage(image: provider, fit: widget.fit, alignment: widget.alignment)),
//         );
//       },
//     );
//   }
