...
class _CategoryCard extends StatelessWidget {
  ...
  String _slugBase(String input) {
    final lower = input.toLowerCase().trim();
    final cleaned = lower.replaceAll(RegExp(r'[^a-z0-9]+'), '_');
    return cleaned.replaceAll(RegExp(r'_+'), '_').replaceAll(RegExp(r'^_|_$'), '');
  }

  String _categoryPosterAsset(String categoryKey, String lc) {
    final k = categoryKey.trim();
    const explicit = <String, String>{
      'Fantasy': 'fantasy',
      'Fairy Tales': 'fairy_tales',
      'Folk Tales/Fables': 'folk_tales_fables',
      'Folk Tales': 'folk_tales_fables',
      'Fables': 'folk_tales_fables',
      'Adventure': 'adventure',
      'Adventures': 'adventure',
      'Moral': 'moral',
      'Moral Stories': 'moral',
    };
    final base = explicit[k] ?? _slugBase(k);
    final suffix = _langSuffix(lc);
    return 'assets/categories/$base-$suffix.webp';
  }
          ...
          child: AspectRatio(
            ...
            child: Stack(
              ...
              children: [
                Builder(
                  builder: (context) {
                    final assetPath = _categoryPosterAsset(categoryKey, langCode);
                    return Image.asset(
                      assetPath,
                      fit: BoxFit.cover,
