  ...
  void _generateParticles() {
    ...
    _stars = List.generate(44, (_) {
      return _Star(
        x: r.nextDouble(),
        y: r.nextDouble(),
        radius: lerpDouble(1.4, 3.2, r.nextDouble())!,
    ...
    _notes = [
      _Note(x: 0.82, y: 0.20, size: 28),
      _Note(x: 0.90, y: 0.30, size: 24),
      _Note(x: 0.76, y: 0.34, size: 22),
      _Note(x: 0.86, y: 0.42, size: 23),
    ];
  }
...
class _SkyPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Crescent moon (top-right) — slightly larger for visibility
    final moonCenter = Offset(size.width * 0.84, size.height * 0.13);
    final moonR = size.shortestSide * 0.075; // bigger moon
    final moonPaint = Paint()..color = const Color(0xFFF6E7B3).withOpacity(darkStyle ? 0.85 : 0.65);

    // soft moon glow
    final moonGlow = Paint()
      ..color = const Color(0xFFF6E7B3).withOpacity(darkStyle ? 0.22 : 0.16)
      ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 22);
    canvas.drawCircle(moonCenter, moonR * 1.35, moonGlow);

    // draw crescent by subtracting a shifted circle using blend mode
    final layerRect = Rect.fromCircle(center: moonCenter, radius: moonR * 1.8);
    canvas.saveLayer(layerRect, Paint());
    canvas.drawCircle(moonCenter, moonR, moonPaint);
    final cutPaint = Paint()..blendMode = BlendMode.clear;
    canvas.drawCircle(moonCenter.translate(moonR * 0.42, -moonR * 0.10), moonR * 0.95, cutPaint);
    canvas.restore();
