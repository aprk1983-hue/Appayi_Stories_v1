...
class _StoryPlayerScreenState extends State<StoryPlayerScreen> with RouteAware {
  ...
  Story? _story;
  bool _loading = true;
  String? _errorMessage; // Shows error UI if load fails
  ...
  void initState() {
    ...
    // NEW: Start the robust loading sequence (Cold Start Fix)
  ...
  // --- NEW: WAITS FOR AUTH BEFORE LOADING ---
  Future<void> _startLoadingSequence() async {
    // 1. Brief pause to let Flutter Engine/Nav settle
    await Future.delayed(const Duration(milliseconds: 500));
    
    // 2. Wait for Auth to settle (Important for Cold Start!)
  ...
  @override
  void didPush() { isStoryPlayerOnTop.value = true; }
  @override
  void didPopNext() { isStoryPlayerOnTop.value = true; }
  @override
  void didPushNext() { isStoryPlayerOnTop.value = false; }
  @override
  void didPop() { isStoryPlayerOnTop.value = false; }
  ...
  Future<void> _share() async {
    ...
    String idToShare = _storyDocId.isNotEmpty ? _storyDocId : widget.storyId;
    
    // Check for global shareId
    if (_currentStorySnap != null && _currentStorySnap!.exists) {
      final data = _currentStorySnap!.data();
      if (data != null && data.containsKey('shareId')) {
        final sid = data['shareId'];
        if (sid != null) {
          idToShare = sid.toString();
        }
      }
    }

    final String encodedId = Uri.encodeComponent(idToShare);
    final String link = 'https://share.appayistories.com/?storyId=$encodedId';

    try {
      final box = context.findRenderObject() as RenderBox?;
      await Share.share(
        'Check out "${_story!.title}" on Audio Story App!\n$link',
        subject: _story!.title,
  ...
  Future<void> _load() async {
    ...
    while (attempts < 5) { // 5 retries
      bool success = await _tryLoadSingleAttempt();
      if (success) return; 
      
      attempts++;
      print("Retrying load story (${attempts}/5)...");
      if (attempts < 5) {
        await Future.delayed(const Duration(milliseconds: 2000));
      }
    }
    ...
    setState(() {
      _loading = false;
      _errorMessage = "Could not find story #${widget.storyId}.\nPlease check your internet connection.";
    });
  }

  Future<bool> _tryLoadSingleAttempt() async {
    try {
      final storiesCol = FirebaseFirestore.instance.collection('stories');
      final rawId = widget.storyId.trim();
      DocumentSnapshot<Map<String, dynamic>>? snap;

      // 1. Try 'shareId' as Integer (Priority)
      if (snap == null) {
        final int? asInt = int.tryParse(rawId);
        if (asInt != null) {
          final q = await storiesCol.where('shareId', isEqualTo: asInt).limit(1).get(const GetOptions(source: Source.serverAndCache));
          if (q.docs.isNotEmpty) snap = q.docs.first;
        }
      }

      // 2. Try 'shareId' as String
      if (snap == null) {
         final q = await storiesCol.where('shareId', isEqualTo: rawId).limit(1).get(const GetOptions(source: Source.serverAndCache));
         if (q.docs.isNotEmpty) snap = q.docs.first;
      }

      // 3. Try 'storyNo' (Legacy Fallback)
      if (snap == null) {
        final int? asInt = int.tryParse(rawId);
        if (asInt != null) {
           final q = await storiesCol.where('storyNo', isEqualTo: asInt).limit(1).get();
           if (q.docs.isNotEmpty) snap = q.docs.first;
        }
      }

      // 4. Try Direct Path
      if (snap == null && rawId.contains('/')) {
        try {
      ...
      _descriptionStr = (data['description'] ?? data['shortDescription'] ?? data['desc'])?.toString();
      final ts = data['createdAt'];
      if (ts is Timestamp) _createdAtTs = ts;

      _coverHttps = await _resolveUrl(story.coverImageUrl);
      _coverLocalPath = (_coverHttps != null && _coverHttps!.isNotEmpty)
          ? await OfflineStoryStore.instance.resolveLocalCoverPath(storyId: snap.id, remoteUrl: _coverHttps!)
          : null;
      ...
      for (final item in _typedAudioScript) {
        if (item.type == 'audio') {
          final remoteUrl = await _resolveUrl((item.audioUrl ?? '').trim());
          if (remoteUrl.isNotEmpty) {
            try {
              final uri = await OfflineStoryStore.instance.resolvePlayableAudioUri(
                storyId: snap.id, remoteUrl: remoteUrl, segmentIndex: playlistIndex,
              );
          ...
          playlist.add(SilenceAudioSource(duration: Duration(milliseconds: item.pauseDurationMs ?? 2000), tag: item.text));
          playlistIndex++;
        }
      }
      ...
      try { snap.reference.update({'views': FieldValue.increment(1)}); } catch (_) {}
      _refreshNeighbors();
  ...
  Future<void> _refreshNeighbors() async {
    ...
    Query<Map<String, dynamic>> q = FirebaseFirestore.instance.collection('stories').where('category', isEqualTo: cat);
    if (lang.isNotEmpty) q = q.where('language', isEqualTo: lang);
    q = q.orderBy('createdAt', descending: true);

    try {
      final olderSnap = await q.startAfterDocument(snap).limit(1).get(); 
      final newerSnap = await q.endBeforeDocument(snap).limit(1).get();
      if (!mounted) return;
      setState(() {
  ...
  void _openStoryById(String id) {
    _audioPlayer.stop();
    Navigator.pushReplacement(context, storyPlayerRoute(id));
  }

  void _wirePromptListeners() {
    _indexSub = _audioPlayer.currentIndexStream.listen((index) {
      if (index == null || _typedAudioScript.isEmpty || index >= _typedAudioScript.length) return; 
  ...
  Future<void> _triggerRewards() async {
    if (_story == null) return;
    final rewards = await GamificationService.instance.handleStoryCompleted(_story!.id, _story!.category ?? '');
  ...
  Future<String> _resolveUrl(String path) async {
    ...
    if (p.startsWith('http://')) return 'https://${p.substring(7)}';
    if (p.startsWith('gs://')) {
      try {
  ...
  Future<Map<String, String?>> _getPreferredChildData(User user) async {
    ...
    if (data != null) {
      final childData = (data['child'] is Map) ? data['child'] as Map<String, dynamic> : null;
      final photo = (childData?['photoUrl'] ?? data['profileImageUrl'])?.toString();
      final nick = (childData?['nickName'] ?? data['childNickname'])?.toString();
      if (nick != null && nick.isNotEmpty) preferredDisplayName = nick;
      if (photo != null && photo.isNotEmpty) preferredPhotoUrl = photo;
    }
    preferredDisplayName ??= user.email ?? 'User';
    return {'displayName': preferredDisplayName, 'photoUrl': preferredPhotoUrl};
  }
  ...
  Future<void> _postComment() async {
    ...
    try {
      final childData = await _getPreferredChildData(user);
      await FirebaseFirestore.instance.collection('stories').doc(_story!.id).collection('comments').add({
        'uid': user.uid,
  ...
  Future<void> _addToPlaylist() async {
    ...
    final itemsCol = FirebaseFirestore.instance.collection('users').doc(user.uid).collection('playlists').doc('default').collection('items');
    try {
      await itemsCol.doc(s.id).set({
        'title': s.title, 'coverImageUrl': s.coverImageUrl, 'category': s.category, 'addedAt': FieldValue.serverTimestamp(),
      }, SetOptions(merge: true));
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Added to playlist')));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Could not add to playlist: $e')));
    }
  }
  ...
  void _showCommentsModal() {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _CommentsBottomSheet(storyRef: FirebaseFirestore.instance.collection('stories').doc(_story!.id), commentsCount: _commentsCount),
    );
  }

  Future<void> _refreshOfflineStatus() async {
    if (_story == null) return;
    final sid = _storyDocId.isNotEmpty ? _storyDocId : widget.storyId;
    if (sid.isEmpty) return;
    bool allAudioPresent = true;
    int playlistIndex = 0;
    for (final item in _typedAudioScript) {
      if (item.type == 'audio') {
        final remote = await _resolveUrl((item.audioUrl ?? '').trim());
        if (remote.isEmpty) { allAudioPresent = false; } 
        else {
          final ok = await OfflineStoryStore.instance.hasAudio(storyId: sid, remoteUrl: remote, segmentIndex: playlistIndex);
          if (!ok) allAudioPresent = false;
        }
        playlistIndex++;
      } else if (item.type == 'prompt') playlistIndex++;
    }
    final coverLocal = (_coverHttps != null && _coverHttps!.isNotEmpty)
        ? await OfflineStoryStore.instance.resolveLocalCoverPath(storyId: sid, remoteUrl: _coverHttps!) : null;
    if (!mounted) return;
    setState(() { _offlineAvailable = allAudioPresent; _coverLocalPath = coverLocal; });
  }

  Future<void> _downloadStoryForOffline() async {
    if (_story == null) return;
    if (_offlineDownloading) return;
    final sid = _storyDocId.isNotEmpty ? _storyDocId : widget.storyId;
    if (sid.isEmpty) return;
    final List<Map<String, dynamic>> audioJobs = [];
    int playlistIndex = 0;
    for (final item in _typedAudioScript) {
      if (item.type == 'audio') {
        final remote = await _resolveUrl((item.audioUrl ?? '').trim());
        if (remote.isNotEmpty) audioJobs.add({'index': playlistIndex, 'url': remote});
        playlistIndex++;
      } else if (item.type == 'prompt') playlistIndex++;
    }
    final hasCover = _coverHttps != null && _coverHttps!.isNotEmpty;
    final int totalSteps = audioJobs.length + (hasCover ? 1 : 0);
    if (totalSteps == 0) return;
    if (!mounted) return;
    setState(() { _offlineDownloading = true; _offlineProgress = 0.0; });
    int completed = 0;
    void updateOverall(double fileProgress) {
      if (!mounted) return;
      setState(() { _offlineProgress = ((completed + fileProgress) / totalSteps).clamp(0.0, 1.0); });
    }
    try {
      if (hasCover) {
        await OfflineStoryStore.instance.downloadCover(storyId: sid, remoteUrl: _coverHttps!, onProgress: (r, t) { final p = (t > 0) ? (r / t).clamp(0.0, 1.0) : 0.0; updateOverall(p); });
        completed++; updateOverall(0);
      }
      for (final job in audioJobs) {
        final int idx = job['index'] as int; final String url = job['url'] as String;
        await OfflineStoryStore.instance.downloadAudio(storyId: sid, remoteUrl: url, segmentIndex: idx, onProgress: (r, t) { final p = (t > 0) ? (r / t).clamp(0.0, 1.0) : 0.0; updateOverall(p); });
        completed++; updateOverall(0);
      }
      if (!mounted) return;
      setState(() { _offlineDownloading = false; _offlineProgress = 1.0; });
      await _refreshOfflineStatus();
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Story is ready to play offline.')));
    } catch (e) {
      if (!mounted) return;
      setState(() { _offlineDownloading = false; _offlineProgress = 0.0; });
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Offline download failed: $e')));
    }
  }

  Future<void> _showOfflineOptions() async {
    final sid = _storyDocId.isNotEmpty ? _storyDocId : widget.storyId;
    if (sid.isEmpty) return;
        ...
        return SafeArea(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Container(width: 40, height: 4, margin: const EdgeInsets.only(bottom: 12), decoration: BoxDecoration(color: Colors.white24, borderRadius: BorderRadius.circular(999))),
                ListTile(
                  leading: const Icon(Icons.delete_outline_rounded, color: Colors.white),
                  title: const Text('Remove offline download', style: TextStyle(color: Colors.white)),
                  subtitle: const Text('Free up device storage', style: TextStyle(color: Colors.white70)),
                  onTap: () async {
                    Navigator.of(context).pop();
                    try {
                      await OfflineStoryStore.instance.deleteStoryDownloads(sid);
                      await _refreshOfflineStatus();
                      if (mounted) ScaffoldMessenger.of(this.context).showSnackBar(const SnackBar(content: Text('Offline download removed.')));
                    } catch (e) {
                      if (mounted) ScaffoldMessenger.of(this.context).showSnackBar(SnackBar(content: Text('Could not remove offline download: $e')));
                    }
                  },
                ),
              ],
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          Padding(
            ...
            child: _offlineDownloading
                ? Center(
                    child: SizedBox(
                      width: 22,
                      height: 22,
                      child: CircularProgressIndicator(
                        value: (_offlineProgress > 0 && _offlineProgress < 1)
                            ? _offlineProgress
                            : null,
                        strokeWidth: 2.4,
                        color: Colors.white,
                ...
                : IconButton(
                    tooltip: _offlineAvailable ? 'Offline options' : 'Download for offline',
                    icon: Icon(
                      _offlineAvailable
                          ? Icons.download_done_rounded
                          : Icons.download_rounded,
                      color: _offlineAvailable ? Colors.lightBlueAccent : Colors.white,
                    ),
                    onPressed: () {
                      if (_offlineAvailable) {
                        _showOfflineOptions();
                      } else {
                        _downloadStoryForOffline();
                      }
                    },
                  ),
          ),
      ...
      body: StreamBuilder<ParentalSettings>(
        ...
        builder: (context, settingsSnap) {
          ...
          if (_loading || !settingsSnap.hasData) return const Center(child: AppLoader());
          if (_story == null) return Center(child: Text('Story not found', style: TextStyle(color: onBg)));
                        ...
                        children: [
                          Positioned.fill(
                            child: (_coverLocalPath != null &&
                                    ...
                                    File(_coverLocalPath!).existsSync())
                                ? Image.file(
                ...
                StreamBuilder<PlayerState>(
                  ...
                  builder: (context, snap) {
                    ...
                    return Padding(
                      ...
                      child: _buildControlsCard(playing: playing),
                    );
                  },
                ),
