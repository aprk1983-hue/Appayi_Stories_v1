...
//     // Home (0) and Profile (4) are always accessible
//     if (i == 0) {
//       setState(() => _index = 0);
//       return;
//     }
...
//     // Middle tabs: Search, Categories, Playlists require subscription
//     if (!_hasSubscription) {
...
//             children: [
//               if (!onTop && _index != 4) const _MiniPlayerDock(),
//               _IPhoneBottomBar(
//                 selectedIndex: _index,
//                 onItemSelected: _onNavItemSelected,
//                 items: _navItems,
//                 activeColor: const Color(0xFF5AC8FA), // light blue selected
...
class _MainTabsState extends State<MainTabs> {
  ...
  // Track access status (paid subscription OR trial)
  ...
  final List<Map<String, dynamic>> _navItems = [
    {'icon': Icons.home_rounded, 'label': 'Home', 'requiresAccess': false},
    {'icon': Icons.search_rounded, 'label': 'Search', 'requiresAccess': true},
    {
    ...
    {'icon': Icons.person_rounded, 'label': 'Profile', 'requiresAccess': false},
  ];
  ...
  void initState() {
    ...
    // Listen to subscription changes
    SubscriptionService().subscriptionStatus.listen((hasSubscription) async {
      // Update access status (subscription OR trial)
  ...
  Future<void> _checkAccessStatus() async {
    try {
      ...
      debugPrint('Error checking access status: $e');
      if (mounted) {
        setState(() {
          _hasAccess = false;
  ...
  void _onNavItemSelected(int i) async {
    // Home tab (index 0) - Always accessible
    if (i == 0) {
      setState(() => _index = 0);
      return;
    }
    ...
    // For Search, Categories, Playlists tabs (indices 1, 2, 3) - Check access
    final requiresAccess = _navItems[i]['requiresAccess'] as bool;

    if (requiresAccess) {
      // Check if user has access (paid OR trial)
      if (!_hasAccess && !_isLoading) {
        // Show paywall dialog with trial info
        final result = await showPremiumSubscribeDialog(
          context,
          remainingTrialDays: _remainingTrialDays,
        );
        if (result == true) {
          // User subscribed, wait for subscription to be processed
          // The subscription listener will update _hasAccess
          return;
        }
  ...
  @override
  Widget build(BuildContext context) {
    // Show loading indicator while checking subscription/trial
    if (_isLoading) {
        ...
        child: Container(
          ...
          child: Row(
            children: List.generate(items.length, (i) {
              ...
              final icon = items[i]['icon'] as IconData;
              final label = (items[i]['label'] as String);
              final requiresAccess = items[i]['requiresAccess'] as bool;

              // Determine if this tab should be "locked"
              final isLocked = requiresAccess && !hasAccess && !isSelected;

              // Check if user is in trial period
              final isInTrial =
                  requiresAccess && !hasAccess && remainingTrialDays > 0;
...
class _IPhoneTabItemState extends State<_IPhoneTabItem> {
  ...
  Widget build(BuildContext context) {
    Color color;
    if (widget.isLocked) {
      color = Colors.grey.shade600; // Locked tab color
    ...
    return GestureDetector(
      behavior: HitTestBehavior.opaque,
      onTap: widget.isLocked ? null : widget.onTap, // Disable tap if locked
      onTapDown: (_) => setState(() => _pressed = true),
      onTapCancel: () => setState(() => _pressed = false),
      onTapUp: (_) => setState(() => _pressed = false),
      child: AnimatedScale(
          ...
          child: Column(
            ...
            children: [
              Stack(
                children: [
                  Icon(widget.icon, size: 24, color: color),
                  if (widget.isLocked && !widget.selected)
                    Positioned(
                      right: -4,
                      top: -4,
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          shape: BoxShape.circle,
                          border: Border.all(
                              color: _getBgColor(context), width: 1.5),
                        ),
                        child: const Icon(
                          Icons.lock,
                          size: 10,
                  ...
                  if (widget.isInTrial && !widget.selected && !widget.isLocked)
                    Positioned(
                      right: -6,
                      top: -6,
                      child: Container(
