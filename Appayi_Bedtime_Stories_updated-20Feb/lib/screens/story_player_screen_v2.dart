...
class _StoryPlayerScreenState extends State<StoryPlayerScreen> with RouteAware {
  ...
  bool _isStoryLocked = false;
  int? _shareIdForGate;
  String _languageForGate = 'en';
  ...
  void initState() {
    ...
    // ✅ Use the NEW SubscriptionService
  ...
  void _listenSubscriptionStatus() {
    ...
    // ✅ Use the NEW SubscriptionService
    _subStatusSub = SubscriptionService().subscriptionStatus.listen((v) {
      if (!mounted) return;
      setState(() {
        _isSubscribedUser = v;
        if (v) _isStoryLocked = false;
      });
    });
  }
  ...
  //   _subStatusSub?.cancel();
  //   _subStatusSub =
  //       SubscriptionAccessService.instance.watchIsSubscribed().listen((v) {
  //     if (!mounted) return;
  //     setState(() {
  //       _isSubscribedUser = v;
  //       // If user just subscribed, unlock the current story UI immediately.
  //       if (v) _isStoryLocked = false;
  //     });
  //   });
  // }

  int? _shareIdFromCurrent() {
    // Prefer explicit shareId field, else fall back to storyNo
    final data = _currentStorySnap?.data();
    final share =
        _coerceInt(data?['shareId'] ?? data?['share_id'] ?? data?['shareID']);
    return share ?? _storyNoFromData(data) ?? _story?.storyNo;
  }

  String _languageFromCurrent() {
    final data = _currentStorySnap?.data();
    return _langFromData(data, fallback: _story?.language) ??
        (_story?.language ?? 'en');
  }

  Future<void> _applySubscriptionGateIfNeeded() async {
    final shareId = _shareIdFromCurrent();
    final lang = _languageFromCurrent();
    _shareIdForGate = shareId;
    _languageForGate = lang;

    if (_isSubscribedUser) {
      if (mounted) setState(() => _isStoryLocked = false);
      return;
    }

    final canPlay = await SubscriptionAccessService.instance
        .canPlayStory(language: lang, shareId: shareId);
    if (!mounted) return;

    if (!canPlay) {
      setState(() => _isStoryLocked = true);
      // Show premium dialog once the story is loaded.
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) showPremiumSubscribeDialog(context);
      });
    } else {
      setState(() => _isStoryLocked = false);
    }
  }
  ...
  Future<void> _share() async {
    ...
    if (ps.childMode) {
      final ok =
          await gate.requireParentPin(context, reason: 'Share this story');
      if (!ok) return;
    }
  ...
  Future<void> _load() async {
    ...
    setState(() {
      _loading = false;
      _errorMessage =
          "Could not find story #${widget.storyId}.\nPlease check your internet connection.";
    });
  }
  ...
  Future<bool> _tryLoadSingleAttempt() async {
    try {
      ...
      if (snap == null) {
        ...
        if (asInt != null) {
          final q = await storiesCol
              .where('shareId', isEqualTo: asInt)
              .limit(1)
      ...
      if (snap == null && rawId.contains('/')) {
        try {
          ...
          if (direct.exists)
            snap = direct as DocumentSnapshot<Map<String, dynamic>>;
        } catch (_) {}
      }
      ...
      if (snap == null) {
        final q3 =
            await storiesCol.where('slug', isEqualTo: rawId).limit(1).get();
      ...
      final String? langOverride =
          _langFromData(data, fallback: story.language);
      _descriptionStr =
          (data['description'] ?? data['shortDescription'] ?? data['desc'])
              ?.toString();
      ...
      _coverLocalPath = (_coverHttps != null && _coverHttps!.isNotEmpty)
          ? await OfflineStoryStore.instance
              .resolveLocalCoverPath(storyId: snap.id, remoteUrl: _coverHttps!)
          : null;

      final currentMedia = AppAudioService.handler.mediaItem.valueOrNull;
      final currentStoryId =
          (currentMedia?.extras?['storyId'] ?? currentMedia?.id)?.toString();
      ...
      for (final itemDynamic in story.audioScript) {
        if (itemDynamic is Map) {
          final item =
              StoryScriptItem.fromJson(itemDynamic as Map<String, dynamic>);
      ...
      for (final item in _typedAudioScript) {
        if (item.type == 'audio') {
          ...
          if (remoteUrl.isNotEmpty) {
            try {
              final uri =
                  await OfflineStoryStore.instance.resolvePlayableAudioUri(
                storyId: snap.id,
                remoteUrl: remoteUrl,
                segmentIndex: playlistIndex,
              );
          ...
          playlist.add(SilenceAudioSource(
      ...
      unawaited(_applySubscriptionGateIfNeeded());
      _syncMediaMeta();
      ...
      if (!alreadyLoaded) {
        // If locked (non-subscriber), do not auto-play.
        final shareId = _shareIdFromCurrent();
        final lang = _languageFromCurrent();
        if (!_isSubscribedUser) {
          final canPlay = await SubscriptionAccessService.instance
  ...
  Future<void> _refreshNeighbors() async {
    ...
    try {
      ...
      setState(() {
        _prevStoryId =
            newerSnap.docs.isNotEmpty ? newerSnap.docs.first.id : null;
        _nextStoryId =
            olderSnap.docs.isNotEmpty ? olderSnap.docs.first.id : null;
      });
      _syncMediaMeta();
    } catch (_) {
      if (mounted) setState(() {});
  ...
  void _wirePromptListeners() {
    _indexSub = _audioPlayer.currentIndexStream.listen((index) {
      if (index == null ||
          _typedAudioScript.isEmpty ||
          index >= _typedAudioScript.length) return;
      final current = _typedAudioScript[index];
      setState(() =>
          _currentPrompt = current.type == 'prompt' ? current.text : null);
    });
  ...
  Future<void> _triggerRewards() async {
    if (_story == null) return;
    final rewards = await GamificationService.instance
        .handleStoryCompleted(_story!.id, _story!.category ?? '');
  ...
  void _listenRealtime(DocumentReference<Map<String, dynamic>> storyRef) {
    ...
    _commentsCountSub =
        storyRef.collection('comments').snapshots().listen((qs) {
  ...
  Future<Map<String, String?>> _getPreferredChildData(User user) async {
    ...
    if (data != null) {
      ...
      final photo =
          (childData?['photoUrl'] ?? data['profileImageUrl'])?.toString();
      final nick =
  ...
  Future<void> _postComment() async {
    ...
    if (!settings.commentsEnabled) {
      final ok =
          await gate.requireParentPinOnce(context, reason: 'Post a comment');
      if (!ok) return;
    }
    try {
      final childData = await _getPreferredChildData(user);
  ...
  Future<void> _refreshOfflineStatus() async {
    ...
    for (final item in _typedAudioScript) {
      if (item.type == 'audio') {
        ...
        if (remote.isEmpty) {
          ...
          final ok = await OfflineStoryStore.instance.hasAudio(
              storyId: sid, remoteUrl: remote, segmentIndex: playlistIndex);
    ...
    final coverLocal = (_coverHttps != null && _coverHttps!.isNotEmpty)
        ? await OfflineStoryStore.instance
            .resolveLocalCoverPath(storyId: sid, remoteUrl: _coverHttps!)
        : null;
    if (!mounted) return;
    setState(() {
      _offlineAvailable = allAudioPresent;
      _coverLocalPath = coverLocal;
    });
  }
  ...
  Future<void> _downloadStoryForOffline() async {
    ...
    for (final item in _typedAudioScript) {
      if (item.type == 'audio') {
        ...
        if (remote.isNotEmpty)
          audioJobs.add({'index': playlistIndex, 'url': remote});
        playlistIndex++;
      } else if (item.type == 'prompt') playlistIndex++;
    }
    ...
    void updateOverall(double fileProgress) {
      ...
      setState(() {
        _offlineProgress =
            ((completed + fileProgress) / totalSteps).clamp(0.0, 1.0);
      });
    }

    try {
      if (hasCover) {
        await OfflineStoryStore.instance.downloadCover(
            storyId: sid,
            remoteUrl: _coverHttps!,
            onProgress: (r, t) {
              final p = (t > 0) ? (r / t).clamp(0.0, 1.0) : 0.0;
              updateOverall(p);
            });
        completed++;
        updateOverall(0);
      ...
      for (final job in audioJobs) {
        final int idx = job['index'] as int;
        final String url = job['url'] as String;
        await OfflineStoryStore.instance.downloadAudio(
            storyId: sid,
            remoteUrl: url,
            segmentIndex: idx,
            onProgress: (r, t) {
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Story is ready to play offline.')));
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _offlineDownloading = false;
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('Offline download failed: $e')));
    }
  }
              ...
              children: [
                ...
                ListTile(
                  ...
                  onTap: () async {
                    ...
                    try {
                      await OfflineStoryStore.instance
                          .deleteStoryDownloads(sid);
                      await _refreshOfflineStatus();
                      if (mounted)
                        ScaffoldMessenger.of(this.context).showSnackBar(
                            const SnackBar(
                                content: Text('Offline download removed.')));
                    } catch (e) {
                      if (mounted)
                        ScaffoldMessenger.of(this.context).showSnackBar(
                            SnackBar(
                                content: Text(
                                    'Could not remove offline download: $e')));
                    }
                  },
                ),
              ],
          ...
          child: Row(
            ...
            children: [
              ...
              _MiniCtl(
                ...
                size: 46,
                onTap: (_prevStoryId != null)
                    ? () => _openStoryById(_prevStoryId!)
                    : null,
              ...
              _MiniCtl(
                ...
                onTap: () async {
                  if (playing) {
                    ...
                    if (_isStoryLocked && !_isSubscribedUser) {
              ...
              _MiniCtl(
                ...
                size: 46,
                onTap: (_nextStoryId != null)
                    ? () => _openStoryById(_nextStoryId!)
                    : null,
    ...
    return Scaffold(
      ...
      body: StreamBuilder<ParentalSettings>(
        ...
        builder: (context, settingsSnap) {
          ...
          if (_story == null)
            return Center(
                child: Text('Story not found', style: TextStyle(color: onBg)));
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Text('${_fmtInt(_views)} views',
                        style: TextStyle(
                            color: Colors.white.withOpacity(0.75),
                            fontSize: 18)),
                    const SizedBox(width: 18),
                    Text(_fmtTsDdMmmYyyy(_createdAtTs),
                        style: TextStyle(
                            color: Colors.white.withOpacity(0.75),
                            fontSize: 18)),
                  ],
                ...
                const SizedBox(height: kVGapL),
                Text(
                  (_descriptionStr != null &&
                          _descriptionStr!.trim().isNotEmpty)
                ...
                _ComposerCard(
                  ...
                  onPickEmoji: (s) => setState(() {
                    _commentCtrl.text = (_commentCtrl.text +
                ...
                if ((_story?.category?.isNotEmpty ?? false) &&
                    _createdAtTs != null) ...[
                  const SizedBox(height: 24),
                  Text(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
              color: border.withOpacity(isPrimary ? 0.9 : 0.55),
...
class _NestedRepliesList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      stream: commentRef.collection('replies').orderBy('createdAt').snapshots(),
      builder: (context, snap) {
                          ...
                          PopupMenuItem<String>(
                            ...
                            child: Row(
                              children: const [
                                Icon(Icons.delete_outline,
                                    color:
                                        _StoryPlayerScreenState.kAccentOrange,
                                    size: 20),
                                SizedBox(width: 8),
                                Text('Delete',
                                    style: TextStyle(
...
class __ReplyInputModalState extends State<_ReplyInputModal> {
  ...
  Future<void> _postReply() async {
    ...
    if (!settings.commentsEnabled) {
      final ok =
          await gate.requireParentPinOnce(context, reason: 'Post a reply');
      if (!ok) return;
    }
    ...
    try {
      ...
      await widget.storyRef
        ...
        'uid': user.uid,
        'displayName':
            child['displayName'] ?? (user.displayName ?? user.email ?? 'User'),
