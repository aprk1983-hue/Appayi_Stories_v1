...
class _HomeScreenState extends State<HomeScreen>
  ...
  // --- CHANGED: DEEP LINKS LISTENER ---
  Future<void> _initDeepLinks() async {
    _appLinks = AppLinks();

    // 1. Check if app was opened with a link (Cold Start)
    try {
    ...
    // 2. Listen for links while app is in background (Warm Start)
  ...
  void _handleDeepLink(Uri uri) {
    ...
    // This is what the updated StoryPlayerScreen sends.
    final paramId = uri.queryParameters['storyId'];
    if (paramId != null && paramId.isNotEmpty) {
      Navigator.push(context, storyPlayerRoute(paramId));
      return;
    }

    // Strategy 2: Check for Path Segment (/story/123)
    // Backup in case you use different link formats later.
    if (uri.pathSegments.contains('story')) {
      final index = uri.pathSegments.indexOf('story');
      if (index + 1 < uri.pathSegments.length) {
        final id = uri.pathSegments[index + 1];
        if (id.isNotEmpty) {
          Navigator.push(context, storyPlayerRoute(id));
        }
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    // UPDATED COLORS: Bright Neon Blue and Bright Neon Pink
    final Color nameColor = (g == 'male' || g == 'boy')
        ? const Color(0xFF00C3FF) // Bright Neon Blue
        : (g == 'female' || g == 'girl')
            ? const Color(0xFFFF00FF) // Bright Neon Pink (Magenta)
